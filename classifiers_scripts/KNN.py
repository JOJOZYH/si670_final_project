...
for i in range(1, num_hours + 1):
    ...
    joblib.dump(pipeline.named_steps['classifier'], f'../outputs/knn_hour_{i}.joblib')
...
plt.title('Combined ROC Curves KNN')
...
prediction_df.to_csv('../outputs/knn_pred.csv', index=False)
print("Predicted values have been saved to knn_pred.csv")

with open('../outputs/knn_params.txt', 'w') as f:
