...
plt.title('Combined ROC Curves XGBoost')
plt.legend(loc="lower right")

# Save the ROC plot as xgb.png
plt.savefig('../outputs/xgb.png')
...
with open('../outputs/xgb_params.txt', 'w') as f:
    for idx, params in enumerate(best_params):
        f.write(f"Hour {idx + 1} Best Parameters: {params}\n")

print("Best parameters for each hour have been saved to xgb_params.txt")
print("ROC curves have been saved as xgb.png")
