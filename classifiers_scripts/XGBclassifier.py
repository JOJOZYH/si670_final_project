...
#%% XGBoost Classifier with Nested Cross-Validation and Progress Indicator
...
param_grid = {
    ...
    'classifier__learning_rate': [0.01, 0.05, 0.1, 0.02]
}
...
for i in range(1, num_hours + 1):  # Adjust the range based on your data
    ...
    inner_cv = 3  # Number of inner folds for GridSearchCV
    ...
    # Initialize a progress bar for outer CV
    print(f"Starting Nested CV for hour {i} with 5 outer folds and 3 inner folds...")
    for fold, (train_idx, valid_idx) in enumerate(tqdm(outer_cv.split(X_train, y_train), 
                                                        total=outer_cv.get_n_splits(), 
                                                        desc=f'Hour {i} Outer CV Folds')):
        ...
        # Optional: Print progress per outer fold
        # tqdm.write(f"Hour {i} Fold {fold + 1}: F1 Score = {f1:.2f}, ROC-AUC = {roc_auc:.2f}")
    ...
    # Store the best model from the last fold (you can choose to store all best models if needed)
    best_models[i - 1]['XGBoost'] = grid_search.best_estimator_
    ...
    # Output the best parameters and test metrics for the current hour
    # print(f"\nHour {i} hours after:")
