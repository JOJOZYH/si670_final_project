...
#%%
"""Support Vector Classifier (SVC) with Nested Cross-Validation"""
...
for i in range(1, 13):
    ...
    pipeline = Pipeline([
        ...
        ('classifier', SVC(probability=True, random_state=1))  # SVC model with probability estimates
    ])
    ...
    # Store best parameters from the last outer fold
    best_params_per_hour.append(grid_search.best_params_)
    print(f"Best parameters for {i} hour(s) after: {grid_search.best_params_}")
...
# Output the best parameters for each hour
for idx, params in enumerate(best_params_per_hour):
    print(f"Hour {idx+1} Best Parameters: {params}")